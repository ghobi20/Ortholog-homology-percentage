...
makeblastdb(file = organism_protein_db, dbtype = "prot", verbose = FALSE) #Si quieres ver el reporte del comando, modifica verbose a TRUE
...
fetch_endpoint_POST <- function(server, request, data, content_type='application/json'){
    r = POST(paste(server, request, sep = ""), content_type(con_type_sequence), accept("application/json"), body = data)
...
for (i in seq_along(perc_ids_df$Gene_id)) {
